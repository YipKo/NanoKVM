    ...
    settings: {
      ...
      network: {
        ...
        wifi: {
          ...
          ipConfig: 'IP 配置',
          ipConfigDescription: '为此 Wi-Fi 连接选择 DHCP 或设置静态地址',
          ...
          identity: '身份 / 用户名',
          authentication: '认证方式',
          innerAuthentication: '内部认证',
          anonymousIdentity: '匿名身份（可选）',
          caCert: 'CA 证书路径（可选）',
          domainSuffixMatch: '域名后缀匹配（可选）',
          clientCert: '客户端证书路径',
          privateKey: '私钥路径',
          privateKeyPasswd: '私钥密码（可选）',
          eapPeap: '受保护的 EAP (PEAP)',
          eapTtls: '隧道 TLS (TTLS)',
          eapTls: 'TLS（证书）',
          eapPwd: 'PWD',
          eapLeap: 'LEAP',
        ...
        ethernet: {
          title: '有线网络',
          description: '配置 IP 信息和可选的 802.1X 认证',
          connect: '配置有线网络',
          connectDesc: '为 {{iface}} 配置 IP 信息，并在需要时启用 802.1X 认证',
          ipConfig: 'IP 配置',
          ipConfigDescription: '为此有线接口选择 DHCP 或设置静态地址',
          authTitle: '802.1X 认证',
          authDescription: '仅当你的有线网络需要用户名、密码或证书认证时才启用',
          addressPlaceholder: '静态 IP 地址（例如 192.168.1.10）',
          gatewayPlaceholder: '网关（可选）',
          passwordUnchanged: '不修改',
          privateKeyPasswdUnchanged: '不修改',
          off: '已禁用',
          enterprise: '企业 / 802.1X',
          active: '802.1X 已启用',
          inactive: '802.1X 已配置，等待认证',
          failed: '配置有线网络 802.1X 失败，请重试。'
        },
        tls: {
